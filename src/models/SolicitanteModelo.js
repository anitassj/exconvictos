...
class SolicitanteModelo {
    ...
    async guardarDatos(solicitanteForm) {
        const { tipo, patente, anio, idMarca, idModelo, nombre, apellido, email, celular } = solicitanteForm;
        const sql = 'INSERT INTO solicitante_form (tipo, patente, anio, id_marca, id_modelo, nombre, apellido, email, celular) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
